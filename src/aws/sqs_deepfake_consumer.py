...
Examples:
  ...
  export QUEUE_URL="https://sqs.us-east-1.amazonaws.com/XXXXXX/deepfake-queue"
  ...
  python sqs_deepfake_consumer.py \\
    --queue-url "https://sqs.us-east-1.amazonaws.com/XXXXXXX/deepfake-queue" \\
  ...
  python sqs_deepfake_consumer.py --run-once
    ...
    parser.add_argument('--queue-url', type=str,
                       help='AWS SQS queue URL for receiving messages (or use QUEUE_URL env var)')
    parser.add_argument('--result-queue-url', type=str,
                       help='AWS SQS queue URL for sending results (or use RESULT_QUEUE_URL env var)')
    ...
    result_queue_url = args.result_queue_url or os.getenv('RESULT_QUEUE_URL')
    ...
    try:
        ...
        logger.info(f"📍 Input Queue: {queue_url}")
        if result_queue_url:
            logger.info(f"📍 Result Queue: {result_queue_url}")
